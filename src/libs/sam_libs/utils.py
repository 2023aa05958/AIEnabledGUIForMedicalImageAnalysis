...
def draw_image(image_rgb, masks, xyxy, probs, labels):
    # Handle empty results - use proper checks for different data types
    def is_empty(obj):
        if obj is None:
            return True
        if hasattr(obj, 'size') and obj.size == 0:  # numpy array
            return True
        if hasattr(obj, '__len__') and len(obj) == 0:  # list/tuple
            return True
        return False
    
    if (is_empty(masks) or is_empty(xyxy) or is_empty(probs) or is_empty(labels)):
        print("⚠️  No detections to draw, returning original image")
        return image_rgb
    ...
    if isinstance(masks, list):
        ...
        if len(masks) > 0:
            ...
            # If no masks, return original image
            print("⚠️  No masks to process, returning original image")
            return image_rgb
    
    # Convert xyxy to numpy array if it's a list
    if isinstance(xyxy, list):
        if len(xyxy) > 0:
            xyxy = np.array(xyxy)
        else:
            print("⚠️  No bounding boxes to process, returning original image")
            return image_rgb
    ...
    # Ensure masks are boolean and not empty
    if hasattr(masks, 'size') and masks.size > 0:
        masks_bool = masks.astype(bool)
    else:
        print("⚠️  Empty masks array, returning original image")
        return image_rgb
